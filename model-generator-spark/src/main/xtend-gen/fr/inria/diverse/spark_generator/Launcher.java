...
import fr.inria.diverse.spark_generator.DMGenMeatamodelGenerator;
...
public class Launcher {
  ...
  public void main(final String... args) {
    try {
      ...
      String _optionValue = commandLine.getOptionValue(Launcher.HDFS_MASTER);
          ...
          public Resource createResource(final URI uri) {
            final XMIResourceImpl result = new XMIResourceImpl(uri) {
              ...
              public URIConverter getURIConverter() {
                return new HadoopURIConverterImpl(Launcher.this.fsConfiguration);
              }
            };
      ...
      String _optionValue_1 = commandLine.getOptionValue(Launcher.E_PACKAGE_CLASS);
      this.ePackage = _optionValue_1;
      String metamodelUrl = commandLine.getOptionValue(Launcher.METAMODEL_URL);
      Launcher.LOGGER.info("Start loading the packages");
      if (((!Objects.equal(this.ePackage, null)) && (!this.ePackage.isEmpty()))) {
        ClassLoader _classLoader = Launcher.class.getClassLoader();
        final Class<?> clazz = _classLoader.loadClass(this.ePackage);
        Method _method = clazz.getMethod("init");
        final Object importedPackage = _method.invoke(null);
      }
      ...
      Launcher.LOGGER.info("Finish loading the packages");
      String _optionValue_2 = commandLine.getOptionValue(Launcher.HBASE_MASTER);
      this.hbaseMaster = _optionValue_2;
      String dmgenUrl = commandLine.getOptionValue(Launcher.DMGEN_URL);
  ...
  public Options configureOptions() {
    ...
    final Option metamodel = OptionBuilder.create(Launcher.METAMODEL_URL);
    metamodel.setLongOpt(Launcher.METAMODEL_URL_LONG);
    ...
    final Option hbase = OptionBuilder.create(Launcher.HBASE_MASTER);
    hbase.setLongOpt(Launcher.HBASE_MASTER_LONG);
    ...
    final Option hdfs = OptionBuilder.create(Launcher.HDFS_MASTER);
    hdfs.setLongOpt(Launcher.HDFS_MASTER_LONG);
    ...
    final Option dmgen = OptionBuilder.create(Launcher.DMGEN_URL);
    dmgen.setLongOpt(Launcher.DMGEN_URL_LONG);
    ...
    final Option package_ = OptionBuilder.create(Launcher.E_PACKAGE_CLASS);
    package_.setLongOpt(Launcher.E_PACKAGE_CLASS_LONG);
