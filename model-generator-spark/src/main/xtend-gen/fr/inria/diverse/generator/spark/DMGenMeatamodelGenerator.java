...
import fr.inria.diverse.generator.spark.DMGenGenerationConfig;
  ...
  public void generate(final Resource resource) {
    try {
      try {
        ...
        EdgesGen _edgesGen = new EdgesGen(this.config, resource);
        JavaRDD<Boolean> _mapPartitions_1 = _mapPartitions.<Boolean>mapPartitions(_edgesGen);
        long _count = _mapPartitions_1.count();
        boolean _greaterThan_1 = (_count > 1);
        if (_greaterThan_1) {
          _xifexpression_1 = 0;
        } else {
          _xifexpression_1 = 1;
        }
        final int exitCode = _xifexpression_1;
